defmodule CallService.CircuitBreaker do
  @moduledoc """
  ...
  ## Usage:
      ...
      CircuitBreaker.call(:redis, fn ->
        RedisClient.get_active_call(call_id)
      end)
  ...
  @fuses %{
    redis: :call_redis_fuse,
    mongodb: :call_mongodb_fuse,
    http: :call_http_fuse,
    kafka: :call_kafka_fuse
  }
  ...
  defp fuse_options(service) do
    config = Application.get_env(:call_service, :circuit_breaker, %{})
  ...
  defp emit_circuit_open_telemetry(service) do
    :telemetry.execute(
      [:call_service, :circuit_breaker, :open],
      %{count: 1},
      %{service: service}
    )
  end
